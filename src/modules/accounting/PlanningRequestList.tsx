...
import { REQ_PHASES, ccPhaseLabel, type ReqPhase } from "./costCenterStore";
import { loadPlanRequests, planTopicLabel } from "./planningStore";
  ...
  return (
    <div className="p-crm">
      ...
      <div className="crm-main">
        <CostCenterSide active="plan-requests" />
        <div className="crm-content">
          <div className="subbar">
            <div className="company-pick"><Building size={15} />{T("คำขอดำเนินการ (Planning)", "Planning action requests")}</div>
            <div className="u-spacer" />
            <div className="fields" onClick={() => nav("/accounting/cost-center/planning/requests/new?topic=PLAN")}><Plus size={16} />{T("สร้างคำขอ", "New request")}</div>
          </div>
              ...
              <table className="data-grid">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    <tr key={r.code} style={{ cursor: "pointer" }} onClick={() => nav(`/accounting/cost-center/planning/requests/${encodeURIComponent(r.code)}`)}>
                      <td className="docno">{r.code}</td>
                      <td>{planTopicLabel(r.topic, thai)}</td>
