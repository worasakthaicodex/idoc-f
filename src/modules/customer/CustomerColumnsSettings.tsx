...
import { getColumns, setColumns, availableColumns, DEFAULT_COLUMNS, SPECIAL_COLS } from "./customerColumnConfig";
import "./customer.css";

export default function CustomerColumnsSettings() {
  const nav = useNavigate();
  const { t } = useTranslation();
  const session = getSession();
  const [used, setUsed] = useState<string[]>(getColumns());
  const [initial] = useState<string[]>(() => getColumns());
  const dragKey = useRef<string | null>(null);
  const [overUsed, setOverUsed] = useState(false);
  const [overAvail, setOverAvail] = useState(false);

  const label = (k: string) =>
    k === "code" ? t("customer.col.code") : k === "status" ? t("customer.col.status") : t(`custFields.${k}`, { defaultValue: k });
  const tag = (k: string) => (SPECIAL_COLS.includes(k) ? t("custColumns.special") : t("custColumns.field"));

  const add = (k: string) => { if (!used.includes(k)) setUsed([...used, k]); };
  const remove = (k: string) => setUsed(used.filter((x) => x !== k));
  const save = () => { setColumns(used); nav("/customer/settings"); };
  const dirty = used.length !== initial.length || used.some((k, i) => k !== initial[i]);

  const avail = availableColumns().filter((k) => !used.includes(k));
        ...
        <div className="crm-content">
          <div className="toolbar">
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
              <div className="tbtn" style={{ fontSize: 12.5 }} onClick={() => setUsed([...DEFAULT_COLUMNS])}>{t("custColumns.reset")}</div>
            </div>
          </div>
          ...
          <div className="crm-body">
            {!allowed ? (
              ...
              <>
                <div className="set-head">{t("custColumns.title")}</div>
                <div className="set-sub">{t("custColumns.dragHint")}</div>
                ...
                <div className="ff-cols">
                  <div className={`ff-panel${overUsed ? " over" : ""}`}
                    ...
                    onDrop={dropOnUsed()}>
                    <div className="ff-head">{t("custColumns.shown")} <span className="ff-count">{used.length}</span></div>
                    <div className="ff-list">
                      {used.length === 0 && <div className="set-hint" style={{ padding: 8 }}>{t("custColumns.emptyShown")}</div>}
                      {used.map((k, i) => (
